...
function mostrar_regiones(){
    ...
    var regiones = "<option value='' selected='selected'>Seleccione una región</option>"
    for (var i = 0; i < chile.regiones.length; ++i) {
        var nombre_region = chile.regiones[i].nombre;
        regiones += "<option value='" + nombre_region+ "'>" + nombre_region+"</option>";
    }
    document.getElementById("region").innerHTML = regiones;
}

function encontrar_indice(name){
    for (var i = 0; i < chile.regiones.length; ++i) {
        if(name == chile.regiones[i].nombre){
            return i;
        }
    }
    return -1;
...
function mostrar_comunas(id){
    var k = encontrar_indice(document.getElementById(id).value);   
    var comunas = "<option value='' selected='selected'>Seleccione una comuna</option>";
    if( k!=-1){
        var region = chile.regiones[k]
        for (var i = 0; i < region.comunas.length; ++i) {
            var nombre_comuna = region.comunas[i];
            comunas += "<option value='" + nombre_comuna+ "'>" + nombre_comuna+"</option>";
        }
    }
    document.getElementById("comuna").innerHTML = comunas;
}